...
export class ImagePickerComponent implements OnInit {
	...
	ngOnInit() {
		if (
			(this.plateform.is('mobile') && !this.plateform.is('hybrid')) ||
			this.plateform.is('desktop')
		) {
		...
		Camera.getPhoto({
			...
			.catch((err) => {
				...
				if (err != 'User cancelled photos app') {
					...
					if (this.useFilePicker) {
						// this will only run if this is a desktop device and some how device camera not working
						this.fileInput.nativeElement.click()
					}
				}
			})
